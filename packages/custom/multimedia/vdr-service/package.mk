...
post_install() {
  ...
  if [ "$ENABLE_VDR_PLUGIN_SKINDESIGNER" = yes ]; then
    ...
    cp -PR $VDR_PLUGIN_SKINDESINGER_DIR/libskindesignerapi/libskindesignerapi.so.0.1.0 $INSTALL/usr/lib
    ln -s libskindesignerapi.so.0.1.0 $INSTALL/usr/lib/libskindesignerapi.so.0
    ln -s libskindesignerapi.so.0.1.0 $INSTALL/usr/lib/libskindesignerapi.so
    mkdir -p $INSTALL/usr/config/vdr/themes
