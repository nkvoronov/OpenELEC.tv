...
__tbs-linux-cc_userspace__ = '/storage/.kodi/userdata/addon_data/driver.dvb.tbs-linux-cc/'
...
######################################################################################################
# get tbs-linux-cc supported devices on a system (only name like 101ADD2B-0)
def get_devices_tbs-linux-cc(tbs-linux-cc_log):
  tuners = []
  try:
    for line in open(tbs-linux-cc_log, 'r'):
      line = line.strip()
    ...
    print 'Error reading hdhomerun log file', tbs-linux-cc
  return tuners
...
######################################################################################################
# get tbs-linux-cc supported devices on a system (name, serial number, type)
def get_devices_tbs-linux-cc(mediaclient_e):
  tuners = []
  try:
    ...
    print 'Error getting tbs-linux-cc tuners info'
  return tuners
