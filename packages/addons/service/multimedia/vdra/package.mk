...
addon() {
  ...
  if [ "$ENABLE_VDR_PLUGIN_SKINDESIGNER" = yes ]; then
    ...
    mkdir -p $ADDON_BUILD/$PKG_ADDON_ID/lib
      ...
      cp -P $CAIRO_DIR/.install_pkg/usr/lib/libcairo.so.2.11400.4 $ADDON_BUILD/$PKG_ADDON_ID/lib/libcairo.so.2
      cp -P $CAIRO_DIR/.install_pkg/usr/lib/libcairo-script-interpreter.so.2.11400.4 $ADDON_BUILD/$PKG_ADDON_ID/lib/libcairo-script-interpreter.so.2
